...
import cv2
import numpy as np
#import custom_test_vid_revided
...
class video(QtCore.QObject):
    ...
    def save(self,savePath):
        ...
        fourcc = cv2.VideoWriter_fourcc(*'XVID')    # add backup format option too, or check on startup if this installed.
        self.out = cv2.VideoWriter(self.savePath,fourcc,int(self.fps),(int(self.width),int(self.height)))
        print(type(self.fps),type(self.width),type(self.height))
