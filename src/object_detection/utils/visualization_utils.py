...
def get_bounding_box(
    ...
    max_boxes_to_draw=20):
  """
  This function returns a list of bounding box which had score greater than the threshold.

  return: list of size N containing [left,right,top,bottom,score]
  the left right top bottom are normalized and score is also *100
  """
  l = list()
  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    if scores is None or scores[i] > min_score_thresh:
      ...
      if scores is None:
        ...
        box_to_color_map[box] = int(100*scores[i])
  ...
  for box, score in box_to_color_map.items():
    ...
    (left, right, top, bottom) = (xmin * im_width, xmax * im_width,
                                  ymin * im_height, ymax * im_height)
    l.append([left,right,top,bottom,score])
    #print("(left, right, top, bottom)",(left, right, top, bottom))
